...
namespace PSAP.DAO.INVDAO
{
    public class FrmSpecialWarehouseWarrantDAO
    {
        ...
        public void QuerySpecialWarehouseWarrantHead(DataTable queryDataTable, string beginDateStr, string endDateStr, string reqDepStr, string repertoryNoStr, int warehouseStateInt, string preparedStr, int approverInt, string commonStr, bool nullTable)
        {
            BaseSQL.Query(QuerySpecialWarehouseWarrantHead_SQL(beginDateStr, endDateStr, reqDepStr, repertoryNoStr, warehouseStateInt, preparedStr, approverInt, commonStr, nullTable), queryDataTable);
        }

        /// <summary>
        /// 查询预算外入库单表头的SQL
        /// </summary>
        public string QuerySpecialWarehouseWarrantHead_SQL(string beginDateStr, string endDateStr, string reqDepStr, string repertoryNoStr, int warehouseStateInt, string preparedStr, int approverInt, string commonStr, bool nullTable)
        {
            string sqlStr = " 1=1";
            if (beginDateStr != "")
            {
        ...
        public void QuerySpecialWarehouseWarrantList(DataTable queryDataTable, string specialWarehouseWarrantStr, bool nullTable)
        {
            ...
            sqlStr = string.Format("select INV_SpecialWarehouseWarrantList.*, SW_PartsCode.CodeName, BS_ProjectList.ProjectNo from INV_SpecialWarehouseWarrantList left join SW_PartsCode on INV_SpecialWarehouseWarrantList.CodeFileName = SW_PartsCode.CodeFileName left join BS_ProjectList on INV_SpecialWarehouseWarrantList.ProjectName = BS_ProjectList.ProjectName where 1=1 {0} order by AutoId", sqlStr);
            ...
            for (int i = 0; i < tmpTable.Rows.Count; i++)
            {
                ...
                switch (wState)
                {
                    case 1:
                        if (checkNoApprover)
                        {
                            MessageHandler.ShowMessageBox(string.Format("预算外入库单[{0}]未审批，不可以操作。", DataTypeConvert.GetString(tmpTable.Rows[i]["SpecialWarehouseWarrant"])));
                    ...
                    case 2:
                        if (checkApprover)
                        {
                            MessageHandler.ShowMessageBox(string.Format("预算外入库单[{0}]已经审批，不可以操作。", DataTypeConvert.GetString(tmpTable.Rows[i]["SpecialWarehouseWarrant"])));
                            swwHeadTable.RejectChanges();
                            if (swwListTable != null)
                                swwListTable.RejectChanges();
                            return false;
                    ...
                    //    if (checkSettle)
                    //    {
                    //        MessageHandler.ShowMessageBox(string.Format("预算外入库单[{0}]已经结账，不可以操作。", DataTypeConvert.GetString(tmpTable.Rows[i]["SpecialWarehouseWarrant"])));
                    //        swwHeadTable.RejectChanges();
                    //        if (swwListTable != null)
                    //            swwListTable.RejectChanges();
                    //        return false;
                    ...
                    case 4:
                        ...
                        {
                            MessageHandler.ShowMessageBox(string.Format("预算外入库单[{0}]已经审批中，不可以操作。", DataTypeConvert.GetString(tmpTable.Rows[i]["SpecialWarehouseWarrant"])));
                            swwHeadTable.RejectChanges();
                            if (swwListTable != null)
                                swwListTable.RejectChanges();
                            return false;
            ...
            using (SqlConnection conn = new SqlConnection(BaseSQL.connectionString))
            {
                ...
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        SqlCommand cmd = new SqlCommand("", conn, trans);
                        DateTime serverTime = BaseSQL.GetServerDateTime();
                        cmd.CommandText = string.Format("select SpecialWarehouseWarrant from INV_SpecialWarehouseWarrantHead where WarehouseState = 2 and SpecialWarehouseWarrant in ({0})", swwHeadNoListStr);
                        ...
                        cmd.ExecuteNonQuery();
                        cmd.CommandText = string.Format("Update INV_SpecialWarehouseWarrantHead set WarehouseState=1 where SpecialWarehouseWarrant in ({0})", swwHeadNoListStr);
                        cmd.ExecuteNonQuery();
